      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 border-b border-border/60 pb-6">
        <div>
          <p className="label-track text-gold flex items-center gap-2">
            <MessageSquare size={14} /> FILM DISCUSSION & FEEDBACK
          </p>
          <h3 className="title-card mt-2 text-2xl text-ivory">Thoughts on {projectTitle}</h3>
        </div>
        ...
        <div>
          {user ? (
            ...
            <button
              ...
              className="label-track flex items-center gap-2 bg-charcoal border border-gold/60 px-4 py-2.5 !text-[9px] text-gold hover:bg-gold hover:!text-charcoal transition-all rounded shadow-sm cursor-pointer"
            >
      ...
      <form onSubmit={handleSubmit} className="space-y-4">
        <div>
          <textarea
            ...
            placeholder={
              user
                ? `Leave your reflection or feedback as ${user.name}...`
                : "Sign in with Google above to share your thoughts, critiques, or observations..."
            }
            rows={3}
        ...
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
          <p className="text-[11px] text-muted-foreground">
            Moderated discussion. Comments appear publicly after review.
          </p>

          <button
            type="submit"
            disabled={!user || !commentText.trim() || isSubmitting}
            className="label-track self-end sm:self-auto flex items-center gap-2 bg-gold px-5 py-2.5 !text-[10px] !text-charcoal font-bold hover:bg-gold/90 transition-all rounded disabled:opacity-40 cursor-pointer"
          >
